...
const Navbar = () => {
  ...
  // ✅ Removed: debug useEffect that logged user on every render
  // ✅ Removed: duplicate logout button — UserAvatar already handles logout
  ...
  return (
    <div className="flex justify-between items-center bg-white px-4 py-3 2xl:py-4 sticky z-10 top-0">
      ...
      <div className="flex gap-4 items-center">
        {user ? (
          <>
            {/* ✅ greeting uses username to match the User model */}
            <span className="text-gray-700 font-semibold">
              Hello, {user.username || user.email || "User"}
            </span>
            <NotificationPanel />
            <UserAvatar />
            {/* ✅ No second logout button — UserAvatar dropdown owns logout */}
          </>
        ) : (
